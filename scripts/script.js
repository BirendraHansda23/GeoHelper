document.addEventListener("DOMContentLoaded", function () {
    ...
    if (countryName) {
        ...
        showErrorNotification("No country specified. Please enter a valid country name.");
    }
});
...
async function fetchCountryImage(countryName) {
    ...
    try {
        const response = await fetch(`https://api.pexels.com/v1/search?query=${countryName}&per_page=15`, {
            headers: {
...
if (countryName) {
    ...
    document.getElementById("data-container").innerHTML = `<p class="error">No country specified. Please go back and search for a country.</p>`;
}